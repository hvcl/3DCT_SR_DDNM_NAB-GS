...
class Diffusion(object):
    def __init__(self, args, config, device=None):
        ...
        from functions.svd_ddnm_cascade import ddnm_diffusion, ddnm_plus_diffusion, ddnm_plus_diffusion_with_latent_opt

        self.ddnm_diffusion = ddnm_diffusion
        self.ddnm_plus_diffusion = ddnm_plus_diffusion
        self.ddnm_plus_diffusion_with_latent_opt = ddnm_plus_diffusion_with_latent_opt
            ...
            if getattr(self.args, "stage2_only", False) and "GS0asTempy" not in self.args.image_folder:
                ...
                for level_idx, level in enumerate(levels):
                    ...
                    for i in tqdm.tqdm(range(N), desc=f"Stage{level_idx+1} DDNM per-frame"):
                        ...
                        # save png
                        tvu.save_image(torch.clamp(out_img[0, 0:1], 0.0, 1.0), os.path.join(self.args.image_folder, f"stage2_pngs_lvl{level_idx}", f"stage2_lvl_{level_idx}_pred_{i:02d}.png"))
                ...
                result_denorm, result_denorm_noAddMin, result_norm, result = self.simplified_ddnm_plus_with_latent_opt(model, cls_fn)
    ...
    def simplified_ddnm_plus(self, model, cls_fn):
        ...
        for x_orig, classes in pbar:
            ...
            for i in range(len(Apy)):
                tvu.save_image(
                    torch.clamp(inverse_data_transform(config, Apy[i]), 0.0, 1.0),
                    os.path.join(self.args.image_folder, f"Apy/Apy_{(idx_so_far + i):02d}.png")
                )
                tvu.save_image(
                    torch.clamp(inverse_data_transform(config, x_orig[i]), 0.0, 1.0),
            ...
            tvu.save_image(
                torch.clamp(x[0], 0.0, 1.0), os.path.join(self.args.image_folder, f"pred_{(idx_so_far + j):02d + 1}.png")
            )
            x0_np = torch.clamp(x[0], 0.0, 1.0).detach().cpu().numpy()
            whole_result.append(x0_np)
            np.save(os.path.join(self.args.image_folder, f"pred_{(idx_so_far + j):02d + 1}.npy"), x0_np)
    ...
    def simplified_ddnm_plus_with_latent_opt(self, model, cls_fn):
        """
        Phase 1: svd_based_ddnm_plus 위에서 per-projection latent optimization만 적용.
        - stage2, stage3, temporal smoothing, denorm은 사용하지 않음.
        - ddnm_plus_diffusion의 로직은 그대로 사용하되, 초기 노이즈 x_noise를 최적화.
        """
        from functions.svd_operators import SuperResolution
        num_opt_steps_list = getattr(self.args, "num_opt_steps_search", [50])
        lr_list = getattr(self.args, "lr_search", [0.01])
        
        if not isinstance(num_opt_steps_list, list):
            num_opt_steps_list = [num_opt_steps_list]
        if not isinstance(lr_list, list):
            lr_list = [lr_list]
        
        logging.info(f"num_opt_steps_list: {num_opt_steps_list}")
        logging.info(f"lr_list: {lr_list}")
        # ddnm_plus는 자체 time schedule을 쓰므로 delta_t는 여기서는 쓰지 않음
        # (delta_t는 DDIM 직접 구현 시에만 필요)
        ...
        # --- SuperResolution 연산자 준비 (A_funcs) ---
        assert self.args.deg == "sr_averagepooling"
        deg_scale = int(self.args.deg_scale)
        ...
        sigma_y = float(getattr(self.args, "sigma_y", 0.0))
        eta = float(getattr(self.args, "eta", 0.9))

        results_all = {}
        total_cfg = len(num_opt_steps_list) * len(lr_list)
        cfg_idx = 0

        for num_opt_steps in tqdm.tqdm(num_opt_steps_list, desc="# opt steps"):
            for lr in lr_list:
                cfg_idx += 1
                cfg_name = f"opt_steps={num_opt_steps}, lr={lr}"
                print(f"\n=== Config {cfg_idx}/{total_cfg}: {cfg_name} ===")
                
                cfg_psnr = []
                cfg_ssim = []
                cfg_time = []

                # projection subset 제한 (원하면 몇 장만 검색)
                max_n = getattr(self.args, "max_sample_for_search", -1)
                for pi, batch in enumerate(
                    tqdm.tqdm(test_loader, desc=f"Phase1-{cfg_name}")
                ):
                    if max_n > 0 and pi >= max_n:
                        break

                    # batch["image"]: HR GT (1,1,512,512) 또는 (1,3,512,512)
                    gt = batch["gt_img"].to(self.device) if "gt_img" in batch else batch["image"].to(self.device)
                    gt_model = data_transform(self.config, gt)  # [-1,1]

                    # batch["deg_img"]: LR GT (1,3,128,128) = deg scale 4x | (1,3,64,64) = deg scale 8x
                    y_lr = batch["deg_img"].to(self.device)
                    y_vec = y_lr.reshape(1, -1)  # ddnm_plus_diffusion 인자와 동일

                    # temp_y: ddnm_plus 내부에서 사용하는 full-res guidance (보통 bilinear upsample)
                    temp_y = F.interpolate(
                        y_lr,
                    ...
                    # ✅ BIRD 방식: 초기 x_0 (노이즈 대신 upsampled LR)
                    x_init = A_funcs.A_pinv(y_vec).reshape(
                        1, self.config.data.channels,
                        self.config.data.image_size,
                        self.config.data.image_size
                    ) # min max: 0.0 1.0
                    x_init = data_transform(self.config, x_init) # [-1, 1]

                    t0 = time.time()
                    
                    # ✅ Latent Optimization (BIRD)
                    x_final, metrics = self.ddnm_plus_diffusion_with_latent_opt(
                        x_init,  # 초기 이미지 (upsampled LR)
                        model,
                        self.betas,
                        eta,
                        A_funcs,
                        ...
                        lr=lr,
                        use_sphere_proj=True,  # ✅ BIRD의 핵심
                        pi=pi,
                        gt_img=gt_model,  # ✅ GT for PSNR/SSIM tracking
                    )

                    elapsed = time.time() - t0

                    # 최종 이미지 변환
                    # print("x_final min/max:", x_final.min().item(), x_final.max().item())
                    # x_img = inverse_data_transform(self.config, x_final)  # [0,1]
                    # print("x_img min/max:", x_img.min().item(), x_img.max().item())
                    # exit()
                    x_img = torch.clamp(x_final, 0.0, 1.0)

                    # PSNR/SSIM (from optimization metrics)
                    if 'final_psnr' in metrics and 'final_ssim' in metrics:
                        psnr_val = metrics['final_psnr']
                        ssim_val = metrics['final_ssim']
                        cfg_psnr.append(float(psnr_val))
                        cfg_ssim.append(float(ssim_val))
                        
                        # Print projection progress
                        print(f"  Proj {pi:03d} | PSNR: {psnr_val:.2f} dB | SSIM: {ssim_val:.4f} | Time: {elapsed:.2f}s")
                    
                    cfg_time.append(float(elapsed))

                    # 샘플 저장 ---> recon_f{cfg_name}/, input_lr/, cfg_name/gt/ 형태로 recon만 각각의 directory에 나눠서 저장하게 해줘.
                    recon_dir = os.path.join(
                        self.args.image_folder, f"recon_{cfg_name}"
                    )
                    os.makedirs(recon_dir, exist_ok=True)
                    tvu.save_image(torch.clamp(x_img, 0.0, 1.0), os.path.join(recon_dir, f"recon_{pi:03d}.png"))
                    input_lr_dir = os.path.join(self.args.image_folder, "input_lr")
                    os.makedirs(input_lr_dir, exist_ok=True)
                    tvu.save_image(torch.clamp(batch["deg_img"], 0.0, 1.0), os.path.join(input_lr_dir, f"input_lr_{pi:03d}.png"))
                    if "gt_img" in batch:
                        gt_dir = os.path.join(self.args.image_folder, f"gt")
                        os.makedirs(gt_dir, exist_ok=True)
                        tvu.save_image(torch.clamp(gt, 0.0, 1.0), os.path.join(gt_dir, f"gt_{pi:03d}.png"))

                # config별 평균 결과 정리
                res = {}
                if len(cfg_psnr) > 0:
                    res["avg_psnr"] = float(np.mean(cfg_psnr))
                    res["avg_ssim"] = float(np.mean(cfg_ssim))
                    res["avg_time"] = float(np.mean(cfg_time))
                res["psnr_list"] = cfg_psnr
                res["ssim_list"] = cfg_ssim
                res["time_per_proj"] = cfg_time
                results_all[cfg_name] = res

                print(
                    f"[{cfg_name}] PSNR={res.get('avg_psnr','NA')}, "
                    f"SSIM={res.get('avg_ssim','NA')}, "
                    f"time={res.get('avg_time','NA')}s"
                )

        # 전체 결과 저장
        save_path = os.path.join(self.args.image_folder, "phase1_latent_opt_results.json")
        with open(save_path, "w") as f:
            json.dump(results_all, f, indent=2)
        print(f"Phase1 results saved to {save_path}")
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        for idx, ITER_DATA in enumerate(pbar):
            ...
            for i in range(len(Apy)): ## what is Apy and orig?
                ...
                tvu.save_image(
                    torch.clamp(apy_i_1ch, 0.0, 1.0),
                    os.path.join(self.args.image_folder, f"Apy/Apy_{(idx_so_far + i):02d}.png")
                )
                tvu.save_image(
                    torch.clamp(x_orig_i_1ch, 0.0, 1.0),
            ...
            if "2rounds" in self.args.image_folder:
                ...
                tvu.save_image(torch.clamp(out2_img[0, 0:1], 0.0, 1.0), os.path.join(self.args.image_folder, f"stage2_png_secondround/stage2_refined_{(idx_so_far):02d}.png"))
                np.save(os.path.join(self.args.image_folder, f"stage2_png_secondround/stage2_refined_{(idx_so_far):02d}.npy"), np.clip(out2_img[0,0].detach().cpu().numpy(), 0.0, 1.0))
            ...
            if "2rounds" in self.args.image_folder and int(self.args.deg_scale) == 8:
                ...
                tvu.save_image(torch.clamp(out3_img[0, 0:1], 0.0, 1.0), os.path.join(self.args.image_folder, f"stage3_png_thirdround/stage3_refined_{(idx_so_far):02d}.png"))
                np.save(os.path.join(self.args.image_folder, f"stage3_png_thirdround/stage3_refined_{(idx_so_far):02d}.npy"), np.clip(out3_img[0,0].detach().cpu().numpy(), 0.0, 1.0))
