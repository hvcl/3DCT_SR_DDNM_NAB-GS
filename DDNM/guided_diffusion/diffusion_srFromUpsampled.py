        ...
        for idx, ITER_DATA in enumerate(pbar):
            ...
            if "perProj" in self.args.image_folder:
                ...
                if sigma_y==0.: # noise-free case, turn to ddnm
                    x, _ = self.ddnm_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, cls_fn=cls_fn, classes=classes, config=config)
                else: # noisy case, turn to ddnm+
                    x, _ = self.ddnm_plus_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, sigma_y, cls_fn=cls_fn, classes=classes, config=config)
            ...
            if "perProj" in self.args.image_folder:
                if (idx % 101 == 0): # and "param_search" not in self.args.image_folder:
