...
class Diffusion(object):
    def __init__(self, args, config, device=None):
        ...
        from functions.svd_ddnm_perProj_kvswap import ddnm_diffusion, ddnm_plus_diffusion
    ...
    def _save_stepwise_outputs(
        ...
        if x_ddnm_first is not None:
            ddnm_vis = torch.clamp(x_ddnm_first[0].detach().cpu(), 0.0, 1.0)
            tvu.save_image(ddnm_vis, os.path.join(proj_dir, "ddnm_first.png"))
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        for idx, ITER_DATA in enumerate(pbar):
            ...
            x_ref = ITER_DATA.get("ref_img", None)
            x_before = ITER_DATA.get("before_refine_img", None)
            x_ddnm_first = ITER_DATA.get("ddnm_first_img", None)
            ...
            if x_ddnm_first is not None:
                x_ddnm_first = x_ddnm_first.to(self.device)
            ...
            temp_ref = data_transform(self.config, x_ref) if x_ref is not None else None
            y = y.reshape((b, hwc))
            ...
            os.makedirs(os.path.join(self.args.image_folder, "ddnm_first_png"), exist_ok=True)
            if x_ref is not None:
            ...
            for i in range(len(Apy)): ## what is Apy and orig?
                ...
                if x_before is not None:
                    before_i = torch.clamp(x_before[i].detach().cpu(), 0.0, 1.0)
                    tvu.save_image(
                        before_i[0],
                        os.path.join(self.args.image_folder, f"before_refine_png/before_refine_{(proj_idx_global + i):02d}.png")
                    )
                if x_ddnm_first is not None:
                    ddnm_i = torch.clamp(x_ddnm_first[i].detach().cpu(), 0.0, 1.0)
                    tvu.save_image(
                        ddnm_i[0],
                        os.path.join(self.args.image_folder, f"ddnm_first_png/ddnm_first_{(proj_idx_global + i):02d}.png")
                    )
                if x_ref is not None:
                    ref_i = torch.clamp(x_ref[i].detach().cpu(), 0.0, 1.0)
                    tvu.save_image(
                        ref_i[0],
            ...
            with torch.no_grad():
                if sigma_y==0.: # noise-free case, turn to ddnm -> HERE ##
                    x, x0_preds, x0t_preds, start_step = self.ddnm_diffusion(
                        x, model, self.betas, self.args.eta, A_funcs, y, temp_y=temp_y, ref_y=temp_ref, cls_fn=cls_fn, classes=None, config=config, args=self.args)
                else: # noisy case, turn to ddnm+
                    x, x0_preds, x0t_preds, start_step = self.ddnm_plus_diffusion(
                        x, model, self.betas, self.args.eta, A_funcs, y, sigma_y, temp_y=temp_y, ref_y=temp_ref, cls_fn=cls_fn, classes=None, config=config, args=self.args)
            ...
            if x_orig.size(0) == 1:
                self._save_stepwise_outputs(
                    ...
                    x_orig[0].detach().cpu(),
                    x_deg[0].detach().cpu(),
                    x_ref[0].detach().cpu() if x_ref is not None else None,
                    x_before[0].detach().cpu() if x_before is not None else None,
                    x_ddnm_first[0].detach().cpu() if x_ddnm_first is not None else None,
                )
            x = [inverse_data_transform(config, xi) for xi in x]
            ...
            for j in range(x[0].size(0)):
                ...
                before_source = x_before if x_before is not None else x_deg
                first_source = x_ddnm_first if x_ddnm_first is not None else x_ref

                before_1ch = torch.clamp(before_source[j][0], 0.0, 1.0).to(self.device)
                before_mse = torch.mean((before_1ch - orig_1ch) ** 2)
                before_psnr = 10 * torch.log10(1 / before_mse)
                avg_before_psnr += before_psnr

                if first_source is not None:
                    ref_1ch = torch.clamp(first_source[j][0], 0.0, 1.0).to(self.device)
                    first_mse = torch.mean((ref_1ch - orig_1ch) ** 2)
                    first_psnr = 10 * torch.log10(1 / first_mse)
                    avg_first_psnr += first_psnr
        ...
        if (idx_so_far - idx_init) > 0:
            ...
            if avg_first_psnr > 0:
                print("Total Average First PSNR: %.2f" % (avg_first_psnr / (idx_so_far - idx_init)))
        print("Number of samples: %d" % (idx_so_far - idx_init))
