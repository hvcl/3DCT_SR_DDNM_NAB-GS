...
from guided_diffusion.script_util import create_model_temporal_v2, create_classifier, classifier_defaults, args_to_dict
import random
...
class Diffusion(object):
    ...
    def sample(self, simplified):
        ...
        if self.config.model.type == 'simple': # here 
            ...
            model = create_model_temporal_v2(**config_dict)
            if self.config.model.use_fp16:
                model.convert_to_fp16()
            
            ckpt = "/workspace/DDNM/exp/logs/chestx14/ema_0.9999_300000.pt"
            # ckpt = "/tmp/openai-2024-10-29-01-53-41-404738/ema_0.9999_090000.pt"
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        N_prev = args.N_proj_per_iter // 2
        N_next = args.N_proj_per_iter // 2
        
        for idx, ITER_DATA in enumerate(pbar):
            # x_orig = ITER_DATA["gt_img"]
            # x_orig = x_orig.to(self.device)

            # x_degs = []
            # for i in range(args.N_proj_per_iter):
            #     x_degs.append(ITER_DATA[f"deg_img_{i}"])
            # x_deg = torch.stack(x_degs).to(self.device)

            # if idx==0 or idx==len(test_dataset)-1:
            #     x_deg = ITER_DATA["deg_img"] # [1, 3, 512, 512]
            #     x_deg = x_deg.to(self.device)
            # else:
            #     x_degs = []
            #     for i in range(args.N_proj_per_iter):
            #         x_degs.append(ITER_DATA[f"deg_img_{i}"])
            #     x_deg = torch.cat(x_degs, dim=0).to(self.device)
            
            if idx == 0 or idx == len(test_dataset) - 1:
                x_deg = ITER_DATA["deg_img"] # [1, 3, 512, 512]
                x_orig = ITER_DATA["gt_img"] # [1, 3, 512, 512]
                # x_warped_noise = ITER_DATA["warped_noise"] # [1, 3, 512, 512]

                # save x_deg and x_orig into png files
                # x_deg = Image.fromarray(x_deg[0].permute(1, 2, 0).cpu().numpy().astype(np.uint8))
                # x_orig = Image.fromarray(x_orig[0].permute(1, 2, 0).cpu().numpy().astype(np.uint8))
                # x_deg.save(f"deg_img_{idx:02d}.png")
                # x_orig.save(f"gt_img_{idx:02d}.png")
                # exit()

                x_deg = x_deg.to(self.device)
                x_orig = x_orig.to(self.device)
                # x_warped_noise = x_warped_noise.to(self.device)
            else:
                # 데이터 로딩 및 처리 부분
                x_deg_prevs = [ITER_DATA[f"deg_img_prev_{i}"] for i in range(N_prev, 0, -1) if f"deg_img_prev_{i}" in ITER_DATA]
                # x_deg_prevs = [ITER_DATA[f"deg_img_prev_{i}"] for i in range(1, N_prev + 1) if f"deg_img_prev_{i}" in ITER_DATA]
                x_deg_nexts = [ITER_DATA[f"deg_img_next_{i}"] for i in range(1, N_next + 1) if f"deg_img_next_{i}" in ITER_DATA]
                # x_orig_prevs = [ITER_DATA[f"gt_img_prev_{i}"] for i in range(1, N_prev + 1) if f"gt_img_prev_{i}" in ITER_DATA]
                # x_orig_nexts = [ITER_DATA[f"gt_img_next_{i}"] for i in range(1, N_next + 1) if f"gt_img_next_{i}" in ITER_DATA]
                # x_warped_noise_prevs = [ITER_DATA[f"warped_noise_prev_{i}"] for i in range(N_prev, 0, -1) if f"warped_noise_prev_{i}" in ITER_DATA]
                # x_warped_noise_nexts = [ITER_DATA[f"warped_noise_next_{i}"] for i in range(1, N_next + 1) if f"warped_noise_next_{i}" in ITER_DATA]

                # 데이터 연결
                x_deg = torch.cat(x_deg_prevs + [ITER_DATA["deg_img"]] + x_deg_nexts, dim=0)
                # print("idx:", idx, "x_deg.shape:", x_deg.shape)
                # x_orig = torch.cat(x_orig_prevs + [ITER_DATA["gt_img"]] + x_orig_nexts, dim=0)
                # x_warped_noise = torch.cat(x_warped_noise_prevs + [ITER_DATA["warped_noise"]] + x_warped_noise_nexts, dim=0)

                # 한 번에 device로 이동
                x_deg = x_deg.to(self.device)
                x_orig = ITER_DATA["gt_img"].to(self.device)
                # x_warped_noise = x_warped_noise.to(self.device)
            ...
            # print(f"y.shape: {y.shape}, Apy.shape: {Apy.shape}")
            
            os.makedirs(os.path.join(self.args.image_folder, "Apy"), exist_ok=True)
            os.makedirs(os.path.join(self.args.image_folder, "Orig"), exist_ok=True)
            apy_cur = inverse_data_transform(config, Apy[N_prev]) if y.shape[0] > 1 else inverse_data_transform(config, Apy[0])
            x_orig_cur = inverse_data_transform(config, x_orig[0]) # if y.shape[0] > 1 else inverse_data_transform(config, x_orig[0])
            apy_cur_1ch = apy_cur[0]
            x_orig_cur_1ch = x_orig_cur[0]
            tvu.save_image(
                apy_cur_1ch,
                os.path.join(self.args.image_folder, f"Apy/Apy_{(idx_so_far):02d}.png")
            )
            tvu.save_image(
                x_orig_cur_1ch,
                os.path.join(self.args.image_folder, f"Orig/orig_{(idx_so_far):02d}.png")
            )
            ...
            x = torch.concat([x_noise] * y.shape[0], dim=0).to(self.device)
            ...
            for j in range(x[0].size(0)):
                ...
                orig = inverse_data_transform(config, x_orig[0]) # if y.shape[0] > 1 else inverse_data_transform(config, x_orig[0])
                orig_1ch = orig[0]
                mse = torch.mean((x_1ch.to(self.device) - orig_1ch) ** 2)
                psnr = 10 * torch.log10(1 / mse)
                avg_psnr += psnr

            idx_so_far += 1 # y.shape[0]
