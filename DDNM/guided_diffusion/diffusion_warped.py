...
class Diffusion(object):
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        for x_deg, x_orig, warped_noise, classes in pbar:
            x_deg = x_deg.to(self.device)
            x_orig = x_orig.to(self.device)
            warped_noise = warped_noise.to(self.device)

            y = data_transform(self.config, x_deg)
            # y_ = A_funcs.A(x_orig)
            # check whether y and y_ are the same
            # print(y.size())
            # print(y_.size())
            # exit()
        ...
        #     x_orig = data_transform(self.config, x_orig)
            ...
            # print(x_orig.shape) # [1, 3, 512, 512]
            # extract one channel of x_orig
            # x_orig = x_orig[:,0,:,:].unsqueeze(1)
            ...
            # print(warped_noise.shape) # [1, 3, 128, 128] or [1, 3, 64, 64]
            # print(optical_flow.shape) # [1, 2, 128, 128] or [1, 2, 64, 64]
