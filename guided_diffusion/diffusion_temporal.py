...
from functions.svd_ddnm_temporal import ddnm_diffusion_temporal, ddnm_plus_diffusion_temporal

import torchvision.utils as tvu

from guided_diffusion.models import Model
from guided_diffusion.script_util import create_model_temporal, create_classifier, classifier_defaults, args_to_dict
import random
...
class Diffusion(object):
    ...
    def sample(self, simplified):
        ...
        if self.config.model.type == 'simple': # here 
            ...
            model = create_model_temporal(**config_dict)
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        for f_idx, (x_deg, x_orig, x_prev, x_after, n_cur, n_prev, n_next, classes) in enumerate(pbar):
            x_deg = x_deg.to(self.device)
            x_orig = x_orig.to(self.device)
            
            n_cur = n_cur.to(self.device)
            # if f_idx > 0: x_prev is not zeros
            # if f_idx < len(val_loader) - 1: x_after is not zeros
            if f_idx > 0:
            ...
            if f_idx < len(val_loader) - 1:
                x_after = x_after.to(self.device)
                n_next = n_next.to(self.device)
            else:
                x_after = None
                n_next = None
            # x_prev = x_prev.to(self.device) if x_prev != torch.zeros_like(x_deg) else None
            # x_after = x_after.to(self.device) if x_after != torch.zeros_like(x_deg) else None

            y = data_transform(self.config, x_deg)
            y_prev = data_transform(self.config, x_prev) if x_prev is not None else None
            y_after = data_transform(self.config, x_after) if x_after is not None else None
            ...
            y = y.reshape((b, hwc))
            y_prev = y_prev.reshape((b, hwc)) if y_prev is not None else None
            y_after = y_after.reshape((b, hwc)) if y_after is not None else None
            ...
            #     Apy_prev = A_funcs.A_pinv(y_prev).view(y_prev.shape[0], config.data.channels, self.config.data.image_size,
            #                                     self.config.data.image_size)
            # else:
            #     Apy_prev = None
            
            # if y_after is not None:
            #     Apy_after = A_funcs.A_pinv(y_after).view(y_after.shape[0], config.data.channels, self.config.data.image_size,
            #                                     self.config.data.image_size)
            # else:
            #     Apy_after = None
            ...
            with torch.no_grad():
                if sigma_y==0.: # noise-free case, turn to ddnm -> HERE ##
                    x = torch.clone(n_cur)
                    x, _ = ddnm_diffusion_temporal(
                        x, model, self.betas, self.args.eta, A_funcs, y, y_prev, y_after, n_prev, n_next,
                        cls_fn=cls_fn, classes=classes, config=config)
                else: # noisy case, turn to ddnm+
                    # x, _ = ddnm_plus_diffusion_temporal(x, model, self.betas, self.args.eta, A_funcs, y, sigma_y, cls_fn=cls_fn, classes=classes, config=config)
                    x, _ = ddnm_plus_diffusion_temporal(
                        x, model, self.betas, self.args.eta, A_funcs, y, y_prev, y_after, sigma_y, cls_fn=cls_fn, classes=classes, config=config)
