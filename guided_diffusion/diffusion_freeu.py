...
# from functions.svd_ddnm_freeu import ddnm_diffusion, ddnm_plus_diffusion

import torchvision.utils as tvu

from guided_diffusion.models import Model
from guided_diffusion.script_util import create_model_freeu, create_classifier, classifier_defaults, args_to_dict
import random
...
class Diffusion(object):
    def __init__(self, args, config, device=None):
        ...
        if "lessSteps" in self.args.image_folder:
            from functions.svd_ddnm_freeu_lessSteps import ddnm_diffusion, ddnm_plus_diffusion
        else:
            from functions.svd_ddnm_freeu import ddnm_diffusion, ddnm_plus_diffusion
    ...
    def sample(self, simplified):
        ...
        cls_fn = None
        if self.config.model.type == 'simple': # here 
            ...
            model = create_model_freeu(**config_dict)
            ...
            # set b1, b2, s1, s2 params for freeu
            '''
            b1, b2, s1, s2, thr_tau = self.args.image_folder.split("/")[-3].split("-")
            b1, b2, s1, s2 = float(b1), float(b2), float(s1), float(s2)
            model.set_freeu_params(b1, b2, s1, s2)
            # b1, b2, b3, s1, s2, s3, thr_tau = self.args.image_folder.split("/")[-2].split("-")
            # b1, b2, b3, s1, s2, s3 = float(b1), float(b2), float(b3), float(s1), float(s2), float(s3)
            # model.set_freeu_params(b1, b2, b3, s1, s2, s3)
            self.thr_tau = float(thr_tau.split("_")[-1])
            # model.enable_freeu()
            '''

            '''
            b1, b2, s1, s2 = '1.0-1.0-1.0-1.0' # self.args.image_folder.split("/")[-3].split("-")
            b1, b2, s1, s2 = float(b1), float(b2), float(s1), float(s2)
            model.set_freeu_params(b1, b2, s1, s2)
            model.enable_freeu()

            # print(model.b1, model.b2, model.s1, model.s2)
            # exit()

            # check model's freeu params are set correctly
            assert b1 == model.b1, f"b1 mismatch: {b1} != {model.b1}"
            assert b2 == model.b2, f"b2 mismatch: {b2} != {model.b2}"
            assert s1 == model.s1, f"s1 mismatch: {s1} != {model.s1}"
            assert s2 == model.s2, f"s2 mismatch: {s2} != {model.s2}"
            print(f"FreeU params set: b1={b1}, b2={b2}, s1={s1}, s2={s2}")
            '''

            self.thr_tau = 0

            self.args.freeu_layers = []
            '''
            if "layer" in self.args.image_folder:
                freeu_layers = self.args.image_folder.split("_layer-")[-1].split("/")[0].split(",")
                self.args.freeu_layers = [int(layer) for layer in freeu_layers]
            '''
        ...
        # if "param_search" not in self.args.image_folder:
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        if deg == 'cs_walshhadamard':
            ...
            cutoff_radius = self.args.cutoff_radius  # 예시값, 이미지 크기에 맞게 조정
            A_funcs = HighFreqDegradation(
                config.data.channels,
                config.data.image_size,
                cutoff_radius,
                self.device,
                img_folder=self.args.image_folder
            )
        elif deg == 'highfreq_sigma':
            from functions.highFrequency_deg import HighFreqDegradationSigma
            # 원하는 cutoff_radius(고주파 반경) 값 지정
            cutoff_radius = self.args.cutoff_radius  # 예시값, 이미지 크기에 맞게 조정
            A_funcs = HighFreqDegradationSigma(
                config.data.channels,
                config.data.image_size,
                cutoff_radius,
                self.device
        ...
        args.startStep = int(args.image_folder.split("startStep_")[-1].split("-")[0])
        args.sigma_y = 2 * args.sigma_y #to account for scaling to [-1,1]
        sigma_y = args.sigma_y
        ...
        for idx, ITER_DATA in enumerate(pbar):
            x_deg = ITER_DATA["deg_img"] # [1, 3, 128, 128] or [1, 3, 64, 64] | Cycle>0 -> [1, 3, 512, 512]
            ...
            os.makedirs(os.path.join(self.args.image_folder, "x_deg"), exist_ok=True)
            os.makedirs(os.path.join(self.args.image_folder, "Apy"), exist_ok=True)
            ...
            for i in range(len(Apy)): ## what is Apy and orig?
                ...
                x_deg_i_1ch = inverse_data_transform(config, temp_y[i])[0]
                ...
                tvu.save_image(
                    x_deg_i_1ch,
                    os.path.join(self.args.image_folder, f"x_deg/x_deg_{(idx_so_far + i):02d}.png")
                )
            ...
            # IF n_ensemble > 1, then we need to repeat the y
            # if self.args.n_ensemble > 1:
            #     y = y.repeat(self.args.n_ensemble, 1)
            #     temp_y = temp_y.repeat(self.args.n_ensemble, 1, 1, 1)

            with torch.no_grad():
                if sigma_y==0.: # noise-free case, turn to ddnm -> HERE ##
                    print("Running DDNM...")
                    x, x0_preds, x0t_preds = self.ddnm_diffusion(
                        x, model, self.betas, self.args.eta, A_funcs, y, sigma_y, self.thr_tau, temp_y=temp_y, cls_fn=cls_fn, classes=None, config=config, args=self.args)
                else: # noisy case, turn to ddnm+
                    print("Running DDNM+...")
                    x, x0_preds, x0t_preds = self.ddnm_plus_diffusion(
                        x, model, self.betas, self.args.eta, A_funcs, y, sigma_y, self.thr_tau, temp_y=temp_y, cls_fn=cls_fn, classes=None, config=config, args=self.args)
                    
            x = [inverse_data_transform(config, xi) for xi in x]

            # IF n_ensemble > 1, then we need to average the x, x0_preds, x0t_preds
            if self.args.n_ensemble > 1:
                # if n_ensemble==5-> x.shape: [5, 3, 512, 512], but list (length 1)
                x = [torch.mean(x[0], dim=0, keepdim=True)] #[:1]] # print(x[0].shape): [3, 512, 512]
            
            # print("x[0].shape:", x[0].shape, type(x[0]))
            # exit()

            # print("x[0].shape:", x[0].shape, type(x[0]))
            # print(x0_preds[0].shape, type(x0_preds[0]))
            # print(x0t_preds[0].shape, type(x0t_preds[0]))
            # exit()
            ...
            # if (idx % 20 == 0) or ("param_search" in self.args.image_folder): # and "param_search" not in self.args.image_folder:
            #     # if self.args.n_ensemble > 1:
            #     #     break
