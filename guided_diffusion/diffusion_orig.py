...
class Diffusion(object):
    ...
    def sample(self, simplified):
        ...
        if self.config.model.type == 'simple':
            ...
            # ckpt = "/workspace/ddim/exp/logs/Code test2/ckpt_120000.pth"
            # ckpt = "/workspace/ddim/exp/logs/totensor/ckpt_800000.pth"
            # ckpt = "/workspace/ddim/exp/logs/CheXpert_8/ckpt_100000.pth"
            # ckpt = "/workspace/ddim/exp/logs/CheXpert_8/ckpt_400000.pth"
            ckpt = "/workspace/Diffusion/ddim/exp/logs/totensor/ckpt.pth"
            # ckpt = "/workspace/Diffusion/ddim/exp/logs/CheXpert_8/ckpt.pth"
            ...
            ckpt = "/tmp/openai-2024-10-29-01-53-41-404738/model220000.pt"
    ...
    def simplified_ddnm_plus(self, model, cls_fn):
        ...
        for x_orig, classes in pbar:
            ...
            tvu.save_image(
                x[0], os.path.join(self.args.image_folder, f"pred_{idx_so_far + j + 1}.png")
            )
            whole_result.append(x[0].detach().cpu().numpy())
            np.save(os.path.join(self.args.image_folder, f"pred_{idx_so_far + j + 1}.npy"), x[0].detach().cpu().numpy())
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        for x_orig, classes in pbar:
            ...
            for j in range(x[0].size(0)):
                tvu.save_image(
                    x[0][j], os.path.join(self.args.image_folder, f"pred_{idx_so_far + j}.png")
                )
                whole_result.append(x[0][j].detach().cpu().numpy())
                np.save(os.path.join(self.args.image_folder, f"pred_{idx_so_far + j}.npy"), x[0][j].detach().cpu().numpy())
