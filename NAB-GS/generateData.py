...
def main():
    ...
    volPath = f'./data/{args.ctName}.nii.gz'
    configPath = f'./data/config/{args.config}.yml'
    outputPath = f'./data/pickle_data/{args.outputName}.pickle'

    print(volPath)
    generator(volPath, configPath, outputPath, False)
    print('Finish')
...
def generator(volPath, configPath, outputPath, show=False):
    ...
    if data["noise"] != 0 and data["normalize"]:
        ...
        data["train"]["up_projections"] = projections
...
def downsample_volume(volume, scale_factor=2):
    ...
    volume = sitk.SmoothingRecursiveGaussian(volume, sigma=1.0)
