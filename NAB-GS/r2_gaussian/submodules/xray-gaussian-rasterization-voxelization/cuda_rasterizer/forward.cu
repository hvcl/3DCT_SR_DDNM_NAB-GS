	...
	float* __restrict__ out_color)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// const float power_cutoff = -5.0f;
			// float val = (power > power_cutoff) ? exp(power) : 0.0f;

			// const float power_cutoff = -5.0f;
			// const float power_softness = 0.005f;  // or 0.5 for sharper edge

			// float t = (power - power_cutoff) / power_softness;
			// t = fmin(fmax(t, 0.0f), 1.0f);  // ensure in [0,1]
			// float fade = t * t * (3.0f - 2.0f * t);  // smoothstep

			// float val = exp(power) * fade;

			//! We compute mu to consider integration.
			// const float alpha = con_o.w * mu * val;
			// const float alpha = (con_o.w < 0) ? (con_o.w * exp(power)) : (con_o.w * mu * exp(power));
