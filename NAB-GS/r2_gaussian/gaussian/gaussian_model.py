...
class GaussianModel:
    def setup_functions(self):
        ...
        # self.density_activation = lambda x:  torch.tanh(x)
        # self.density_inverse_activation = inverse_tanh

        # self.density_activation = lambda x: torch.where(x>=0, x, x * 0.0) # x4: 0.09, x8: 0.003 UHRCT: x4: 0.03 x8: 0.003
        # self.density_inverse_activation = lambda y: torch.where(y >= 0, y, y / 0.0)

        self.density_activation = lambda x: torch.where(x>=0, x, x * 0.003) # x4: 0.09, x8: 0.003 UHRCT: x4: 0.03 x8: 0.003
        self.density_inverse_activation = lambda y: torch.where(y >= 0, y, y / 0.003)

        # self.density_activation = lambda x : torch.sin(x)
        # self.density_inverse_activation = lambda y : torch.asin(y)
    ...
    # error based
    # def add_densification_stats(self, viewspace_point_tensor, update_filter, error_weight=None):
    #     # 기존
    #     g = torch.norm(viewspace_point_tensor.grad[update_filter, :2], dim=-1, keepdim=True)

    #     if error_weight is not None:
    #         # error_weight: [N_vis] -> [N_vis,1]
    #         g = g * error_weight[:, None]
