...
class GaussianModel_prelu:
    def setup_functions(self):
        ...
        def density_activation(x, model=self):
            a = model.density_alpha  # learnable α > 0
            return torch.where(x >= 0, x, x * a)

        def density_inverse_activation(y, model=self):
            a = model.density_alpha
            # PReLU의 역함수: y>=0 -> x=y, y<0 -> x=y/α
            return torch.where(y >= 0, y, y / a)

        self.density_activation = density_activation
        self.density_inverse_activation = density_inverse_activation

        self.rotation_activation = torch.nn.functional.normalize

    def __init__(self, scale_bound=None, init_alpha=0.1, learn_alpha=True): # x4: 0.1, x8: 0.001
        ...
        alpha_init = torch.tensor(init_alpha, dtype=torch.float32)
        self._alpha_raw = nn.Parameter(torch.log(torch.exp(alpha_init) - 1.0))
    ...
    def get_scaling(self):
        return self.scaling_activation(self._scaling) # N, 3
    ...
    @property
    def density_alpha(self):
        # 항상 양수 기울기
        return torch.nn.functional.softplus(self._alpha_raw) + 1e-6
    ...
    def training_setup(self, training_args):
        ...
        # ★ density와 완전히 같은 lr/scheduler를 쓰고 싶으면 이렇게
        if self.learn_alpha:
            l.append(
                {
                    "params": [self._alpha_raw],
                    "lr": training_args.alpha_lr_init * self.spatial_lr_scale,
                    "name": "density_alpha",
                }
            )
        ...
        self.alpha_scheduler_args = get_expon_lr_func(
            lr_init=training_args.alpha_lr_init,
            lr_final=training_args.alpha_lr_final,
            max_steps=training_args.density_lr_max_steps,
        )
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "rotation":
                ...
                param_group["lr"] = lr
            if param_group["name"] == "density_alpha":
                param_group["lr"] = self.alpha_scheduler_args(iteration)
    ...
    def _prune_optimizer(self, mask):
        ...
        for group in self.optimizer.param_groups:
            ...
            # xyz/density/scaling/rotation만 point-wise 이므로 mask 적용
            if name not in ["xyz", "density", "scaling", "rotation"]:
                # density_alpha 같은 글로벌 파라미터는 그대로 유지
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        ...
        for group in self.optimizer.param_groups:
            ...
            # xyz/density/scaling/rotation 외의 글로벌 파라미터(density_alpha 등)는 건너뜀
    ...
    def densify_and_prune(
        ...
        prune_mask = ((self.get_density < min_density) & (self.get_density > -min_density)).squeeze()
