...
def readBlenderInfo(path, eval):
    ...
    vol_lr = torch.from_numpy(np.load(meta_data["vol"])).float().cuda()
    vol_up = torch.from_numpy(np.load(meta_data["vol"])).float().cuda()
...
def readNAFInfo(path, eval):
    ...
    for split in splits:
        ...
        lr_up_projs = data["train"]["up_projections"]
        ...
        for i_split in range(n_split):
            ...
            lr_image = lr_up_projs[i_split] * scene_scale
            ...
            # Get intrinsic
            DSD = scanner_cfg['DSD']
            dDetector = scanner_cfg['dDetector']
            nDetector = scanner_cfg['nDetector']
    ...
    vol_lr = torch.from_numpy(data["lr_image"]).float().cuda() #lr_image: LR volume, #upsampled: up LR volume by cubic
    vol_up = torch.from_numpy(data["upsampled"]).float().cuda() #lr_image: LR volume, #upsampled: up LR volume by cubic
    scene_info = SceneInfo(
        train_cameras=train_cam_infos,
        test_cameras=test_cam_infos,
        scanner_cfg=scanner_cfg,
