...
class SuperResolution(A_functions):
    def __init__(self, channels, img_dim, ratio, device): #ratio = 2 or 4
        ...
        self.img_dim = img_dim
        self.channels = channels
        self.y_dim = img_dim // ratio # 512 // 4 = 128, 512 // 8 = 64
        self.ratio = ratio
        A = torch.Tensor([[1 / ratio**2] * ratio**2]).to(device)
        '''
        ratio = 4인 경우
        A는 1x16 크기의 텐서
        self.U_small: 1x1 크기의 텐서
        self.singulars_small: 1 크기의 텐서
        self.V_small: 16x16 크기의 텐서        
        '''
        # if ratio is 8 -> A is 1x64
...
class SRConv(A_functions):
    ...
    def Lambda(self, vec, a, sigma_y, sigma_t, eta):
        # For convolution-based SR, use the Wiener-like filter scaling
        if sigma_t >= a * sigma_y:
    ...
    def Lambda_noise(self, vec, a, sigma_y, sigma_t, eta, epsilon):
        # Noise-aware variant for DDNM+ with proper noise scaling
        d1_t = sigma_t * eta
        d2_t = sigma_t * (1 - eta ** 2) ** 0.5
        
        if a != 0 and sigma_y != 0:
            if sigma_t >= a * sigma_y:
                d1_t = torch.sqrt(sigma_t ** 2 - a ** 2 * sigma_y ** 2)
                d2_t = 0.0
            else:
                d1_t = sigma_t * eta
                d2_t = sigma_t * (1 - eta ** 2) ** 0.5
        
        return vec * d1_t + epsilon * d2_t
...
class Deblurring(A_functions):
    ...
    def singulars(self):
        return self._singulars.repeat(1, 1).reshape(-1)
