...
def ddnm_diffusion(x, model, b, eta, A_funcs, y, temp_y=None, ref_y=None, cls_fn=None, classes=None, config=None, args=None):
    with torch.no_grad():
        ...
        for t_idx, (i, j) in tqdm(enumerate(time_pairs)):
            ...
            if j < i: # normal sampling 
                ...
                # ===== Reference batch for KV swap =====
                ref_base = ref_y if ref_y is not None else temp_y
                noised_ref = at.sqrt() * ref_base + (1 - at).sqrt() * torch.randn_like(ref_base).to(ref_base.device)
                
                # [0] = noised reference, [1] = target xt
                x_concat = torch.cat([noised_ref, xt], dim=0)  # [2, 3, 512, 512]
                
                # timestep도 batch size에 맞게 확장
                t_concat = torch.cat([t, t], dim=0)  # [2]
                
                if cls_fn == None:
                    # model에 concat된 input 전달
                    et_concat = model(x_concat, t_concat)
                    # 실제 prediction은 batch index 1에서만 사용
                    et = et_concat[1:2]  # [1, C, H, W]
                else:
                    classes = torch.ones(x_concat.size(0), dtype=torch.long, device=torch.device("cuda"))*class_num
                    et_concat = model(x_concat, t_concat, classes)
                    et = et_concat[1:2, :3]  # [1, 3, H, W]
                    et = et - (1 - at).sqrt()[0, 0, 0, 0] * cls_fn(x, t, classes[1:2])
                # ====================================================================
...
def ddnm_plus_diffusion(x, model, b, eta, A_funcs, y, sigma_y, temp_y=None, ref_y=None, cls_fn=None, classes=None, config=None, args=None):
    with torch.no_grad():
